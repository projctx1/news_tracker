...
function _getTweets() {
  _getTweets = _asyncToGenerator(
    /*#__PURE__*/_regenerator().m(function* _callee() {
      ...
      try {
        const targets = yield TargetAccount.find({});
        ...
        for (const item of targets) {
          ...
          if (tweets.length > 0) {
            for (const tweetModel of tweets) {
              ...
              const exists = yield Tweet.exists({ TweetId: tweetModel.id });
              if (!exists) {
                yield Tweet.create({
          ...
          yield new Promise(resolve => setTimeout(resolve, 32 * 60 * 1000));
        }
      } catch (error) {
        console.error(" Error fetching tweets:", error.message);
      }

      // Wait 12 hours before running again
      yield new Promise(resolve => setTimeout(resolve, 12 * 60 * 60 * 1000));
    })
  );
